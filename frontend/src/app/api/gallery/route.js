...
export async function POST(request) {
  try {
    ...
    const newFilename = `gallery_${timestamp}_${random}${extension}`;
    const newPath = path.join(process.cwd(), 'public/uploads/gallery', newFilename);
    
    // Move file to final destination
    await fs.rename(file.filepath, newPath);
    
    // Return relative URL for use in frontend
    const fileUrl = `/uploads/gallery/${newFilename}`;
